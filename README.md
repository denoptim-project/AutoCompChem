...
AutoCompChem is available from the [denoptim-project channel in Anaconda](https://anaconda.org/channels/denoptim-project/packages/autocompchem/overview), so it can be installed from within any conda environment as follows
```
...
--<keyword> <value>
```
for those requiring a value. Quotation marks should be used as usual, when the value may contain spaces or other characters that the command line would interprete in unintended ways. E.g., `--<keyword> "<value1> <value2> <value3>"`. Note that ANSI-C quotation may be needed by arguments that contain new line characters, e.g., `--SMARTS $'[#44]~[#17] prefix: list A\n[#6]~[#44] prefix: list B'`.
