...
/**
 * This class is substantially a copy of  CDK's SMARTSQueryTool. Small changes
 * were required for acting on topologically complex molecules
 * Search for 'MF' labels to find changes
 * Please refer to CDK's API for further detail.
 * @author MarcoFoscato (modifications)
 * @author Rajarshi Guha (original)
 */
...
public class SMARTSQueryTool {
     ...
     *
     * @param atomContainer The target molecule
...
//MF
//
// This modification allows to deal with some complex topologies that is
// common in organometallic chemistry whitout hitting the time limit
//
//            allRings = arf.findAllRings(atomContainer);
//
// HARD CODED:
//            arf.setTimeout(maxTimeAllRingFinder);
            arf.setTimeout(10000);
//
// HARD CODED:
//            allRings = arf.findAllRings(atomContainer, maxRingSize);
            allRings = arf.findAllRings(atomContainer, 7);
//MF end
