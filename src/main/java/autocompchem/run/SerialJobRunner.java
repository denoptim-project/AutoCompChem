...
public class SerialJobRunner extends JobsRunner
{
    /**
...
//------------------------------------------------------------------------------
     ...
     * job from the queue.
     * @param master the job that creates this {@link SerialJobRunner}.
     */

    public SerialJobRunner(List<Job> todoJobs, int poolSize, int queueSize, 
    		Job master)
    {
    ...
    public void start()
    {
    	...
    	while (requestedToStart && !weRunOutOfTime())
    	{
    		...
    		if (requestedAction!=null)
    		{
    			...
    			switch (requestedAction.getObject()) 
    			{
					case FOCUSJOB:
					{
						reactionObjectJobs.add(
								(Job) jobRequestingAction.exposedOutput.getNamedData(
    			...
    			ActionApplier.performAction(requestedAction, jobRequestingAction, reactionObjectJobs, 
    					restartCounter.get());
					...
					switch (action.getType())
    				{
						...
						case SKIP:
							...
							synchronized (lock)
			            	{
				    			...
				    			ActionApplier.performAction(requestedAction, jobRequestingAction, 
