...
//------------------------------------------------------------------------------
    		  ...
    		  int restartCounter)
    {
    	...
    	archivePreviousResults(stepsToReRun, restartCounter, 
    			action, jobRequestingAction.getUserDir());
...
//------------------------------------------------------------------------------
			...
			File customUserDir)
    {
		for (Job job : jobs)
		{
			...
			archivePreviousResults(job, restartCounter, action, customUserDir);
		}
    }
			...
			Action action, File customUserDir)
    {
		...
		for (DataArchivingRule rule : action.getJobArchivingRules())
		{
			if (rule.getType().equals(ArchivingTaskType.RENAME_COPY_LAST_SEQUENTIAL))
			{
				...
				{
					idx++;
					String pattern = rule.getPattern().replace(DataArchivingRule.SEQUENTIAL_INDEX, String.valueOf(idx));
