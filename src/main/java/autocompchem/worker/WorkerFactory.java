...
//-----------------------------------------------------------------------------
    ...
    public static Worker createWorker(ParameterStorage params, Job mainJob)
    {
    	...
    	//TODO-gg: add tracking of child jobs? By setParent() we add a reference 
    	// of the parent (P) in the child (C), so C can access P, but P does not
    	// know about C. 
    	// We could add a collection of child job's references in P, so that P
    	// becomes aware of C. Yet, that may require cleaning that collection
    	// once the child job is completed, of keeping it to retain any data
    	// generated by C. Yet this may be too memory demanding, as we keep
    	// data (about C and from the task done by  C) that we do not know if 
